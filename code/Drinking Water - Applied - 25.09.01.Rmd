...
# Application: 50% proportional reduction intervention for all exposures
myintervention <- 0.50
df_intervention <- df %>%
  mutate(
...
# Question:	What's a technical issue with the proportional reduction approach in which exposures for all individuals are reduced by a constant proportion (Hints: think in terms of causal or modeling assumptions, and consider the post-intervention exposures for the participant with the lowest exposures)?
...
# Application: 50% BOUNDED proportional reduction intervention for all exposures
myintervention <- 0.50
df_intervention <- df %>%
  mutate(
...
## Hint: think about regulatory limit values we used above for individual exposures.
...
# In the USA, we know that people living in rural locations often have higher exposure to contaminants in their drinking water than those living in urban areas. This is based on a number of structural factors, but represents a potential driver of health disparities. Here, we want to expand on our prior scenarios by applying hypothetical interventions that result in people from rural areas having reductions in exposures so that they're approximately equal to average exposures among people living in urban areas.
...
# Short answer: Yes, code above shows us that the average level among the rural areas is higher than among urban areas for all exposures
...
# Definition: a "bootstrap sample" is a sample of the analysis units (here, individual rows) with replacement from the analytic dataset
...
gcomp_get_effectmeasures <- function(bootdf_intervention) {
  bootdf_intervention %>%
    summarise(
      observed = mean(obs_biomarker),
      predicted = mean(pred_biomarker),
...
gcomp_draw_bootsample <- function(df, no_resample = FALSE) {
...
gcomp_do_allsteps <- function(df, iter, no_resample = FALSE) {
  ### 1) draw bootstrap sample
  bootdf <- gcomp_draw_bootsample(df, no_resample = no_resample)
  ...
  cross_join(data.frame(bootiter = iter), gcomp_get_effectmeasures(bootdf_intervention))
}
...
pointest <- gcomp_do_allsteps(df, 0, no_resample = TRUE)[, c("abs_change", "pct_change")]
...
```{r EXAMPLE 5 - follow-up questions}
